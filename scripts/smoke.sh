...
mkdir -p "$HOME/.musketeer/registry/tools"
...
mkdir -p "$HOME/.musketeer/registry/tools/loopexec/0.1.0" "$HOME/.musketeer/registry/tools/musketeer/0.1.0"
...
resp=$(curl -s -X POST "http://127.0.0.1:${PORT}/v1/tools/loopexec/run" -H 'content-type: application/json' -d '{"version":"0.1.0","args":{},"cwd":".","env":{},"mode":"json","client":{"name":"smoke"}}')
echo "$resp" | jq -e '.exit_code!=null' >/dev/null
printf 'PASS run response has exit_code\n'
printf 'PASS smoke\n'
